...
/* ─── Field component ────────────────────────── */
function Field({ label, name, type = 'text', placeholder, required, as, options, values, errors, touched, onChange, onBlur }) {
  ...
  return (
    <div className={styles.field}>
      ...
      {as === 'textarea' ? (
        <textarea {...inputProps} rows={4} className={`${inputProps.className} ${styles.textarea}`} />
      ) : as === 'select' ? (
        <select {...inputProps}>
...
/* ─── Contact Form ───────────────────────────── */
const CONTACT_INITIAL = { name: '', email: '', company: '', phone: '', employees: '', message: '' }

function ContactForm() {
  const { values, errors, touched, status, serverMessage, handleChange, handleBlur, handleSubmit } = useForm(
    CONTACT_INITIAL,
    validateContact,
    async (vals) => {
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit} noValidate aria-label="Contact form">
      <div className={styles.formRow}>
        <Field label="Full Name" name="name" placeholder="Anika Reddy" required {...fieldProps} />
        <Field label="Work Email" name="email" type="email" placeholder="anika@company.com" required {...fieldProps} />
      </div>
      <div className={styles.formRow}>
        <Field label="Company" name="company" placeholder="Acme Corp" required {...fieldProps} />
        <Field label="Phone" name="phone" type="tel" placeholder="+91 98765 43210" {...fieldProps} />
      </div>
      <Field
        label="Team Size"
        name="employees"
        ...
        {...fieldProps}
      />
      <Field label="Tell us about your use case" name="message" as="textarea" placeholder="We handle ~10,000 inbound calls per day and need to automate tier-1 support..." required {...fieldProps} />

      {status === 'error' && (
        <p className={styles.serverError} role="alert">{serverMessage}</p>
      )}
      ...
      <p className={styles.formFooter}>No spam. Our team responds within 2 hours on business days.</p>
    </form>
  )
}

/* ─── Demo Form ──────────────────────────────── */
const DEMO_INITIAL = { name: '', email: '', company: '', date: '', time: '', timezone: '' }
...
function DemoForm() {
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit} noValidate aria-label="Demo booking form">
      <Field label="Full Name" name="name" placeholder="Marcus Whitfield" required {...fieldProps} />
      <Field label="Work Email" name="email" type="email" placeholder="marcus@company.com" required {...fieldProps} />
      <Field label="Company" name="company" placeholder="RetailEdge" {...fieldProps} />
      <Field label="Preferred Date" name="date" type="date" required {...fieldProps} />
      <div className={styles.formRow}>
        <Field
      ...
      <p className={styles.formFooter}>30-minute session · You'll receive a calendar invite immediately.</p>
    </form>
  )
}
          ...
          <div className={styles.layout}>
            ...
            <aside className={styles.infoPanel}>
              <div className={styles.infoCard}>
                ...
                <div className={styles.infoItems}>
                  {[
                    ...
                    { icon: '🛟', label: 'Support',  val: 'support@voiceaxis.io' },
              ...
              <div className={styles.infoCard}>
                ...
                {[
                  { city: 'Bangalore', addr: 'Prestige Tech Park, Whitefield, 560066' },
                  { city: 'Singapore', addr: '1 Raffles Place, #20-61, 048616' },
                  { city: 'San Francisco', addr: '101 Mission St, Suite 1750, CA 94105' },
                ].map(({ city, addr }) => (
